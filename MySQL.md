...
2. MyISAM不支持全文索引，InnoDB支持全文索引
3. MyISAM不支持外键，InnoDB支持外键
4. MyISAM只支持表锁，InnoDB还支持行锁
...
### 15.为什么innoDB存储引擎选用B+树，而不是B树

1. B+树的磁盘读写代价更低。内部节点不存放具体信息，相对于B树的内部节点更小。
2. B+树的查询效率更稳定。任何查找都必须从根节点寻找到叶子节点。
3. B+树更易于遍历。B+树的叶子节点连接在一起。
4. B+树更适合基于范围的查询。只需要遍历叶子节点，就可以实现整棵树的遍历。
...
读写分离常⽤代理方式来实现，代理服务器接收应用层传来的读写请求，
然后决定转发到哪个服务器。
主服务器处理写操作以及实时性要求⽐较高的读操作，⽽从服务器处理读操作。
...
延迟原因：
假如一个服务器开放N个连接给客户端，这样有会有⼤大并发的更新操作,
但是从服务器的⾥面读取binlog的线程仅有一个，
当某个SQL在从服务器上执行的时间稍长或者由于某个SQL要进⾏锁表就会导致主服务器的SQL⼤量积压，未被同步到从服务器里。
这就导致了主从不一致，也就是主从延迟。

解决方法：
增加从服务器，这个⽬的还是分散读的压⼒，从而降低服务器负载
